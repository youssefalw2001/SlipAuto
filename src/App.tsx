...
import { Crosshair, Menu, Package, RotateCcw, Trophy, Users, X } from "lucide-react";
                ...
                {NAV.map(n => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        color: page === n.id ? '#ff7040' : '#6060a0',
                        background: page === n.id ? 'rgba(255,77,0,0.08)' : 'transparent',
                      }}
                    >
                      ...
                      {n.id === "crates" && (
                        <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded-full font-bold"
                          style={{ background: '#ff4d00', color: 'white' }}>NEW</span>
                      )}
                    </button>
                  );
                })}
                <button
                  onClick={() => { setShowOnboard(true); setOpen(false); }}
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-5 py-8">
        <AnimatePresence mode="wait">
          <motion.div key={page} variants={pageAnim} initial="initial" animate="animate" exit="exit">
            {page === "yoink"       && <YoinkGame xp={xp} onXPGain={addXP} levelId={levelData.id} />}
            {page === "wheel"       && <SwapWheel />}
            {page === "crates"      && <CrateShop xp={xp} levelId={levelData.id} onXPGain={addXP} />}
      ...
      <footer className="border-t border-y-border py-6">
        <div className="max-w-6xl mx-auto px-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-4 text-[11px]" style={{ color: '#6060a0' }}>
            ...
            <span style={{ color: '#30304a' }}>·</span>
            <button onClick={() => setShowOnboard(true)} className="hover:text-white transition-colors">
              How to Play
            </button>
          </div>
        </div>
